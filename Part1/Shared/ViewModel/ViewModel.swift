...
class ViewModel: ObservableObject {
    ...
    "Now you play the role of a Chinese introducer who introducing intangible cultural heritage . "
    + "Use Chinese."
    + "Only answer the intangible cultural heritage related content"
...
//        let config = APIConfig(apiKey: apiKey, model: model, stream: stream, temperature: temperature, basePrompt: basePrompt)
        let config = APIConfig(model: model, stream: stream, temperature: temperature, basePrompt: basePrompt)
        if self.api == nil {
